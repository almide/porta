...
| Another process's arguments and environment are unreadable | Seatbelt `procargs`/`process-info` denies; Linux `/proc` closed under strict | `escapes.py` "read another process's arguments" |
