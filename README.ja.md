...
ポートを強制しますが、**読み取りは制限しません** — allow-list 専用のため、
「`~/.ssh` と `~/.gnupg` だけ拒否して他は許可」という macOS の挙動を表現できません。
...
| **FS 読み取り** | 制限しません。Landlock は allow-list 専用で、macOS の `~/.ssh`・`~/.gnupg` 拒否に相当する表現がありません |
