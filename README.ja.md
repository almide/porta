...
AI エージェントにマシン上でコマンドを実行させることは、鍵もトークンも
ネットワークも、その手の届く範囲に置くということです。Porta はそのコマンドを
実行しつつ、OS カーネルに実際に止めさせます。書き込み・読み取り・通信・
ソケットを、macOS は Seatbelt、Linux は Landlock＋seccomp で強制する ——
ライブラリのラッパーでも、プロンプトでのお願いでもなく、OS の境界そのものです。
カーネルが表現できない制限は、緩めて実行せず、実行を拒否します（**fail-closed**）。

しかも主張ではなく、証拠で示します。既知の脱獄手法をバイナリに当てる公開
コーパスで、**macOS 12/12・Linux 15/15、突破ゼロ**。負けた行も隠しません
（負けを隠すコーパスはただのブローシャーだから）。詳しくは
[脱獄コーパス](docs/benchmarks/escapes.md)と[脅威モデル](docs/threat-model.md)。

実際の1回の実行 —— 正規の作業は通り、盗み出しと暴走は通らない。

```text
$ porta run sh -v ./work --read-policy strict --timeout 5 -- -c '…'
[agent] 正規の作業              →  ./work/out.txt に書けた
[agent] SSH秘密鍵を読む         →  拒否
[agent] 作業ディレクトリ外に書く →  拒否
# ハングするコマンドは締切で強制終了 → 終了コード 124
```

効いている制限は目に見えません。だから見どころは2つの「拒否」と締切です ——
エージェントが何を試みても、カーネルが止めました。
...
`porta check` はこのホストで何が強制できるかを表示します。どちらも `--json`
を付けると機械可読な形式になります。`explain --json` は実効ポリシー（コマンド、
mount、読み取り、ネットワーク、listen ポート、Unix ソケット、timeout、バックエンド）、
拒否される実行なら `{"refused": "..."}` を返します。`check --json` はホストの各
プリミティブと有無を返します。CI がプロンプト文字列を解析せずポリシーを判定できます。

`porta explain <command> [flags] --save porta.toml` は、そのフラグを
`porta.toml` に書き出します。commit して `porta up` で再実行でき、たどり着いた
起動方法がプロジェクトの設定になります。秘密情報と `-e` の値は意図的に書き出しません。
commit されるファイルはそれらを置く場所ではないからです。
