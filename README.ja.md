...
macOS は `sandbox-exec`、Linux は Landlock を使います。Landlock は書き込みと TCP
ポートを強制します。読み取りは既定ではどちらも開放ですが、Linux では
`--read-policy strict` で閉じられます — 読み取りを、許可した mount と
コマンドの起動に必要なシステムディレクトリだけに限定するので、**ホーム
ディレクトリ配下は一切読めません**。macOS はまだこれを表現できないため、
読み取りを開いたまま実行するのではなくフラグを拒否します。
